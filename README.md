...
- คำขออ่านข้อมูลใช้ CORS `fetch` แบบ hedged retry ก่อน และใช้ JSONP เป็นทางสำรองเมื่อจำเป็น พร้อม cache-buster ใหม่ในแต่ละรอบ
- ระบบใช้ชื่ออย่างเป็นทางการ `WorkLogs Attendance` และมี favicon/ไอคอนสำหรับ Browser, Bookmark และหน้าจอมือถือ
- บันทึกชื่อ `ตำบล / อำเภอ / จังหวัด` จากพิกัดลงฟิลด์ `location_name` และแสดงร่วมกับพิกัดทั้งฝั่งผู้ใช้และแอดมิน
- โครงสร้างชีตเดิมถูกเพิ่มคอลัมน์แบบอัตโนมัติ โดยข้อมูลเก่าที่ยังไม่มีชื่อสถานที่ยังแสดงผลได้ตามปกติ
- รายชื่อพนักงานไม่ส่ง Base64 รูปโปรไฟล์ทั้งชุดอีกต่อไป โดยโหลดเฉพาะรูปที่จำเป็นเมื่อเปิดแก้ไข
- คำขออ่านข้อมูลใช้ CORS `fetch` ก่อน JSONP ทำให้การเปิดรูปยืนยันตัวตนไม่ติด timeout จาก callback ที่ล่าช้า
- รายงานที่ข้อมูลในช่วงวันที่ไม่เกิน 100 รายการสามารถกรองสาขา/พนักงานและเปลี่ยนหน้าในเครื่องได้ทันที
